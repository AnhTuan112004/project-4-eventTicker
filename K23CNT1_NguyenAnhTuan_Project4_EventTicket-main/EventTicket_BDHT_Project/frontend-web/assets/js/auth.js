...
function loginWithGoogle() {
    if (!isConfigured(SOCIAL_AUTH_CONFIG.googleClientId)) {
        setLoginMessage('Vui lòng cấu hình tham số Google Client ID trong tệp login.html để sử dụng tính năng này.');
        return;
    }
...
function loginWithFacebook() {
    if (!isConfigured(SOCIAL_AUTH_CONFIG.facebookAppId)) {
        setLoginMessage('Vui lòng cấu hình Facebook App ID trong tệp login.html để sử dụng đăng nhập Facebook.');
        return;
    }
    if (!window.FB) {
